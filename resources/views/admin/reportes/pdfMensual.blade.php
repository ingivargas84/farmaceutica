...
<body>
    ...
    <table id="datos">
      <tr>
          <td style="width:100%; text-align: left;">___________________________________________________________________</td>
      </tr>
      <tr>
            <td style="font-Size: 24px"><b> Reporte Movimiento Mensual - <?php
            if($mes == 1)
              echo "Enero";
            else if ($mes == 2)
              echo "Febrero";
    ...
    for($i=1; $i<=$dias; $i++)
    {
        if(!empty($warehouse_stock[$i][0])){
        $v += $warehouse_stock[$i][0]->ventas;
        }
        if(!empty($warehouse_stock1[$i][0])){
        $ac += $warehouse_stock1[$i][0]->abono;
        }
        if(!empty($warehouse_stock2[$i][0])){
        $cp += $warehouse_stock2[$i][0]->compras;
        }
        if(!empty($warehouse_stock3[$i][0])){
        $ap += $warehouse_stock3[$i][0]->abonos;
        }
    }
    ...
    <table id="datos">
        ...
        <?php $a = $a + 1;?>
        <tr>
          <td style="width: 20%"><?php echo ($a."/".$mes."/".$anio); ?></td>

          @if(!empty($warehouse_stock[$i][0]))
          @if($warehouse_stock[$i][0]->ventas > 0)
          <td style="width: 20%">Q.<?php echo number_format($warehouse_stock[$i][0]->ventas, 2, ".", ",");?></td>
          @else
          <td style="width: 20%">Q.0.00</td>
          @endif
          @else
          <td style="width: 20%">Q.0.00</td>
          @endif
          ...
          @if(!empty($warehouse_stock1[$i][0]))
          @if($warehouse_stock1[$i][0]->abono > 0)
          <td style="width: 20%">Q.<?php echo number_format($warehouse_stock1[$i][0]->abono, 2, ".", ",");?></td>
          @else
          <td style="width: 20%">Q.0.00</td>
          @endif
          @else
          ...
          @if(!empty($warehouse_stock2[$i][0]))
          @if($warehouse_stock2[$i][0]->compras > 0)
          <td  style="width: 20%">Q.<?php echo number_format($warehouse_stock2[$i][0]->compras, 2, ".", ",");?></td>
          @else
          <td style="width: 20%">Q.0.00</td>
          @endif
          @else
          ...
          @if(!empty($warehouse_stock3[$i][0]))
          @if($warehouse_stock3[$i][0]->abonos > 0)
          <td style="width: 20%">Q.<?php echo number_format($warehouse_stock3[$i][0]->abonos, 2, ".", ",");?></td>
          @else
          <td style="width: 20%">Q.0.00</td>
          @endif
          @else
        ...
        <tr>
         <td><b>Total</b></td>
         <td><b>Q.<?php echo number_format($v, 2, ".", ",");?></b></td>
         <td><b>Q.<?php  echo number_format($ac, 2, ".", ","); ?></b></td>
         <td><b>Q.<?php  echo number_format($cp, 2, ".", ",");?></b></td>
         <td><b>Q.<?php  echo number_format($ap, 2, ".", ",");?></b></td>
         </tr>
    </table>

    <footer>
        <b>Reporte generado por  @foreach($usu as $u)
            {{$u->username}}
          @endforeach el
